        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="panel panel-default">
			  <div class="panel-heading">
				<h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>用户信息列表</h3>
			  </div>
	...
	<script src="${APP_PATH}/jquery/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            function pageQuery(pageno){
            	...
            	$.ajax({
            		type : "POST",
            		url  : "${APP_PATH}/user/queryPageData.do",
            		data : jsonData,
            		beforeSend : function(){
            		...
            		success : function(result){
            			...
            			if(result.success){
            				var dataContent = "";
            				$.each(userPage.data,function(i,user){
            	                dataContent = dataContent + '<tr>';
            	                dataContent = dataContent + '  <td>'+(i+1)+'</td>';
            					dataContent = dataContent + '  <td><input type="checkbox"></td>';
            	                dataContent = dataContent + '  <td>'+user.user_account+'</td>';
            	                dataContent = dataContent + '  <td>'+user.user_name+'</td>';
            					...
            					dataContent = dataContent + '      <button type="button" class="btn btn-primary btn-xs" onclick="editUserById('+user.id+','+pageno+'\)"><i class=" glyphicon glyphicon-pencil"></i></button>';
            					dataContent = dataContent + '	  <button type="button" class="btn btn-danger btn-xs" onclick="deleteUserById('+user.id+',\''+user.user_account+'\')"><i class=" glyphicon glyphicon-remove"></i></button>';
            					dataContent = dataContent + '  </td>';
            	                dataContent = dataContent + '</tr>';
            				});
            			...
            			for(var i = 1;i <= userPage.totalsize;i++){
            				if(i == pageno){
			...
			function deleteUserById(id,user_account){
				layer.confirm("是否要删除"+user_account+",请选择？",  {icon: 3, title:'提示'}, function(cindex){
					$.ajax({
						type  : "POST",
						...
						success  : function(result){
							if(result.success){
								layer.msg("删除"+user_account+"成功", {time:1000, icon:5, shift:6});
								pageQuery(1);
							}else{
			...
			function editUserById(id,pageno){
				window.location.href = "${APP_PATH}/user/editUser.htm?id="+id+"&pageno="+pageno;
			}
