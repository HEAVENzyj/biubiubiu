...
<script type="text/javascript">
    ...
    $("#insertBtn").click(function(){
        window.location.href = "${APP_PATH}/teacher/addTeacher.htm";
    });
    ...
    function pageQuery(pageno){
        ...
        $.ajax({
            type : "POST",
            url  : "${APP_PATH}/teacher/selectLikeTeacher.do",
            data : jsonData,
            beforeSend : function(){
            ...
            success : function(result){
                ...
                var teacherPage = result.data;
                if(result.success){
                    var pageContent = "";
                    $.each(teacherPage.data,function(i,teacher){
                        pageContent = pageContent + '<tr>';
                        pageContent = pageContent + '  <td>'+(i+1)+'</td>';
                        pageContent = pageContent + '  <td><input type="checkbox"></td>';
                        pageContent = pageContent + '  <td>'+teacher.tea_number+'</td>';
                    ...
                    layer.msg("老师信息查询失败！", {time:1000, icon:5, shift:6});
                }
                //生成页码
                var pageContent = "";
                if(pageno > 1){
                ...
                for(var i = 1;i <= teacherPage.totalsize;i++){
                    if(i == pageno){
                ...
                if(pageno < teacherPage.totalsize){
